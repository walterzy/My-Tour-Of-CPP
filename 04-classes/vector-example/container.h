...
public:
    ...
    // (it does not have implementation in the abstract class);
    ...
    virtual ~Container() {} // destructor... this function has an (empty) implementation,
                            // but it can be redefined by subclasses
    
    // without virtual, even if a subclass has implemented its own 'xpto', 
    // if its type is Container, the function below will be called instead.
