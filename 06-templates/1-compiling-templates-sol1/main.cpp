...
int main() {
    ...
    // we just use its object file (.o) and its header file (.h). Thus, the compiler 
    // does not know details about implementations during this compilation.
    ...
    // declarations and definitions during this compilation. We have some solutions for that.
    
    // ### Solution 1 ###
    // Physically move the definitions of the template functions into the header file (.h)
    // cons:
    // It may (or may not!) cause significant code bloat, meaning your executable size may 
    // increase dramatically (or, if your compiler is smart enough, may not; try it and see).
    // That is, each header/source file that includes that header file are going to put 
    // those definitions, increasing the amount of text.
