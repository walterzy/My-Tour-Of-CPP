...
int main() {
    ...
    // Then, inside the same source file, we added a specific line for each possible class 
    // that we think that can be used (and useful) in other codes (see the final lines in vector.cpp)
    // This will cause the compiler to compile these particular types so the associated 
    ...
    // this will cause a compilation error, since we do not have any Vector object with 
    // the class std::string in the source file (see function AvailableTemplateClassesForVector() 
    // in vector.cpp
